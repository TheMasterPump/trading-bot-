"""
FAST STRATEGY ANALYSIS (SAMPLED)
Analyze derniers trades pour identifier la strategie - version rapide
"""
import asyncio
import httpx
import json
...
async def analyze_sampled_activity(days=7, sample_size=3000):
    """Analyze sampled recent trading activity"""

    print("=" * 70, flush=True)
    print(f"FAST STRATEGY ANALYSIS (Sample: {sample_size} transactions)", flush=True)
    print("=" * 70, flush=True)
    ...
    # Sample transactions
    sample_signatures = signatures[:sample_size]
    print(f"\n[*] Analyzing {len(sample_signatures)} sampled transactions...", flush=True)
    ...
    for i, sig_data in enumerate(sample_signatures):
        ...
        if (i + 1) % 100 == 0:
            print(f"[+] Processed {i + 1}/{len(sample_signatures)} transactions ({len(token_trades)} tokens found)", flush=True)
    ...
    print(f"\nPerformance (Last {days} Days - {sample_size} tx sample):", flush=True)
    ...
    for i, r in enumerate(winners, 1):
        dt = datetime.fromtimestamp(r['first_trade']).strftime('%m-%d %H:%M')
...
async def main():
    await analyze_sampled_activity(days=7, sample_size=3000)
